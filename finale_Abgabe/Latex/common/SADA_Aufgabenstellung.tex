\section*{Aufgabenstellung}
Im Rahmen dieses Projektseminars soll basierend auf am Fachgebiet SIM (Fachbereich Informatik) vorhandenen Komponenten ein Ballbot-Roboter nach dem in \cite{ETHZ} beschriebenen Beispiel aufgebaut und in Betrieb genommen werden. Im Einzelnen sind dabei die Arbeitspakete:
\begin{itemize}
	\item Konstruktion der notwendigen mechanischen Baugruppen
	\item Aufbau des Roboters
	\item Inbetriebnahme von Aktorik und Sensorik über On-Board-Rechner
	\item Identifikation der Systemparameter sowie
	\item Modellierung und Regelung
\end{itemize}
zu bearbeiten.

Ziel der Arbeit ist es, einen funktionsfähigen Roboter zu haben, auf dem ein einfaches Balancieren auf der Stelle implementiert ist. Dabei kann die Regelung durchaus von bestehenden Arbeiten übernommen werden. Unabhängig von den letztendlich implementierten Regelkonzepten wird jedoch eine ausführliche Recherche zu den Regelkonzepten aus der Literatur gefordert.


\vspace{0.5cm}
\begin{tabular}{ll}
Beginn: & \SADABegin \\
Ende:   & \SADAAbgabe \\
Präsentation:& \SADASeminar
\end{tabular}

\vspace{1cm}

\begin{tabular}{ll}
\rule{7cm}{0.4pt} \hspace{1cm} & \rule{7cm}{0.4pt} \\
\SADAProf & \SADABetreuer\\
 &\SADABetreuerII\\
 &\SADABetreuerIII
\end{tabular}

\vfill
{\renewcommand{\baselinestretch}{1} % für diesen Abschnitt einfacher Zeilenabstand
\normalsize % anwenden des Zeilenabstandes
\begin{minipage}{0.8\textwidth}
	Technische Universität Darmstadt\\
	\SADAinstitut\\[0.5cm]
%
	Landgraf-Georg-Straße 4\\
	64283 Darmstadt\\
	Telefon \SADAtel\\
	\SADAwebsite
\end{minipage}
\begin{minipage}{0.2\textwidth}
\flushright  % rechtsbündig
\ \\[2.7cm]
\SADAlogo\;
\end{minipage}}

